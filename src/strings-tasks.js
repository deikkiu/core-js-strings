...
function removeFirstOccurrences(str, value) {
  ...
  return index !== -1 ? str.replace(value, '') : str;
}
...
function formatTime(minutes, seconds) {
  return String(minutes)
    ...
    .concat(':')
    .concat(
      String(seconds) > 9 ? String(seconds) : String(seconds).padStart(2, '0')
    );
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    if (vowels.includes(array[i])) count += 1;
  }
...
function isPalindrome(str) {
  ...
  return (
    letters.split('').reverse().join('').toLowerCase() === letters.toLowerCase()
  );
}
...
function findLongestWord(sentence) {
  ...
  let word = firstElement;

  for (let i = 0; i < array.length; i += 1) {
    const element = array[i];

    if (element.length > firstLength) {
      firstLength = element.length;
      word = element;
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    ...
    if (element.toUpperCase() === element) {
      finalArray.push(element.toLowerCase());
    } else {
      finalArray.push(element.toUpperCase());
    }
  }
...
function encodeToRot13(str) {
  const defaultString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const ROT13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const array = [];

  for (let i = 0; i < str.length; i += 1) {
    const index = defaultString.indexOf(str[i]);

    if (index === -1) {
      array.push(str[i]);
    } else {
      array.push(ROT13[index]);
    }
  }
